...
(async () => {
  ...
  await fs.appendFile('leiomyoma-of-uterus-potential-cases.csv', 'patient-id,dob,codes,last-encounter\n');

  for(let patient in patients) {
    try {
      await fs.appendFile('leiomyoma-of-uterus-potential-cases.csv', patient+','+dobs[patient]+',\"'+Array.from(patients[patient]).join(',')+'\",'+lastEncounter[patient].slice(0, -1)+'\n');
    } catch(error) {
      console.log(error);
    }
  }
